...
def __execute(sql, params: dict = None, msg: str = None):
    log.info(sql)
    return __get_client().execute(sql, params=params, with_column_types=True, settings={'max_block_size': 100000})


def __query(sql, params: dict = None, msg: str = None, result_style: str = 'dict'):
    result, columns = __execute(sql, params=params)
...
def __drop(tbl_name):
    sql = """DROP TABLE IF EXISTS {};""".format(tbl_name)
    return __execute(sql, msg='删除表{}失败'.format(tbl_name))


def __truncate(tbl_name):
    sql = """TRUNCATE TABLE IF EXISTS {};""".format(tbl_name)
