...
class DatasetManager:
    ...
    def list_dataset(self, dsid):
        sql = "SELECT * FROM {} WHERE dsid ='{}'".format(const.tbl_sci_dataset, dsid)
        return self.dao.query_sci_dataset(sql)


    def __update_clean_status(self, dsid, status):
        sql = "ALTER TABLE {} UPDATE status='{}' WHERE dsid = '{}'".format(const.tbl_sci_meta, status, dsid)
        dao.update_dim_dataset(sql)

    def showProcess(self, dsid):
        sql = "SELECT * FROM {} WHERE dsid='{}'".format(const.tbl_sci_meta, dsid)
        self.log.info(sql)
        return self.dao.find_dim_dataset(sql)
    ...
    def rename(self, dsid, newName):
        sql = "ALTER TABLE {} UPDATE dsname='{}' WHERE dsid = '{}'".format(const.tbl_sci_meta, newName, dsid)
        dao.update_dim_dataset(sql)
    ...
    def updateOdsbib(self, form):
        arr_keys = [k for k, v in SciDataset().__dict__.items() if type(v) == list] # 列表类型的字段
...
class DatasetCleaner:
    def __init__(self, dsid, userid):
        ...
        self.dao = dao
        sql = "SELECT * FROM {} WHERE dsid ='{}'".format(const.tbl_sci_dataset, dsid)
        self.dataset = self.dao.query_sci_dataset(sql)
    ...
    def __load_dim_config_text(self, config_style, text):
        ...
        # 根据策略，加载词典
        if style == '1':
            result = self.__query_dim_config(const.default_user, text)
            return set(regex.split(result))
        elif style == '2':
            result = self.__query_dim_config(self.userid, text)
            return set(regex.split(result))
        elif style == '3':
            result1 = self.__query_dim_config(const.default_user, text)
