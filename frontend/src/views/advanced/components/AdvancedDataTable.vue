  ...
  <div>
    <vxe-toolbar ref="xToolbar" :refresh="{ query: fetch }" print custom>
      <template v-slot:buttons>
        ...
        <vxe-button @click="filterDataset">字段设置</vxe-button>
...
export default {
  ...
  mounted() {
    this.$bus.$on(this.$api.advanced_dataset_query, dim_ds => {
  ...
  methods: {
    fetch() {
      ...
      return new Promise(resolve => {
        setTimeout(async () => {
          // console.log('tblid', this.dim_ds.tblid)
          let _url = this.$api.advanced_dataset_query + '/' + this.dim_ds.tblid
